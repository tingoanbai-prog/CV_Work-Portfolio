...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function submitNumber() {
        ...
        if (value >= 1 && value <= 25 && !numbers.includes(value)) {
...
function generateUniqueNumbers() {
    const numbers = Array.from({ length: 25 }, (_, i) => i + 1); // Create array [1, 2, ..., 25]
...
function startLottery() {
    ...
    function drawBall(index) {
        ...
        let tempTimer = setInterval(function () {
            aBall[index].innerText = Math.ceil(Math.random() *25 ).toString().padStart(2, '0');
...
function alerta(lotteryNumbers, userNumbers, matchedNumbers) {
    ...
    if (matchedNumbers.length > 1) {
        resultDiv.innerHTML = `
            ...
            <button id="winButton2">挑戰Hard</button>
        `;
        ...
        document.getElementById("winButton2").addEventListener("click", () => {
            window.location.href = "hard.html"; // Replace with the detailed results page URL
        });
    } else {
        resultDiv.innerHTML = `
